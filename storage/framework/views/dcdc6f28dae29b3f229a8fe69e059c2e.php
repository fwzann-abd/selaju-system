...
<?php $component->withAttributes([]); ?>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <form action="<?php echo e(route('admin.sejajan.index')); ?>" method="GET" class="w-full md:max-w-sm">
                <div class="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm shadow-sm focus-within:border-indigo-500 dark:border-slate-700 dark:bg-slate-900">
                    ...
                    <input type="text" name="q" value="<?php echo e($search); ?>" placeholder="Cari nama toko atau pemilik"
                           class="w-full border-none bg-transparent text-sm text-slate-700 placeholder-slate-400 focus:ring-0 dark:text-slate-200"
                           autocomplete="off">
                    <?php if($search): ?>
                        <a href="<?php echo e(route('admin.sejajan.index')); ?>" class="text-xs text-indigo-500 hover:underline">Reset</a>
                    <?php endif; ?>
                </div>
            </form>
            <div class="flex flex-wrap gap-3">
                <a href="<?php echo e(route('admin.sejajan.create')); ?>"
                            ...
                            <tr class="transition hover:bg-slate-50 dark:hover:bg-slate-800/60">
                                <td class="px-6 py-4 text-sm">
                                    <div class="flex items-center gap-3">
                                        <?php if($shop->photo): ?>
                                            <img src="<?php echo e(asset('storage/assets/modules/sejajan/mart/' . $shop->photo)); ?>"
                                        ...
                                        <?php endif; ?>
                                        <div>
                                            <div class="font-semibold text-slate-800 dark:text-slate-100"><?php echo e($shop->name); ?></div>
                                            <div class="text-xs text-slate-500 dark:text-slate-400"><?php echo e($shop->slug); ?></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">
                                    <?php echo e($shop->participant?->name ?? '-'); ?>

                                </td>
                                <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">
                                    <?php echo e(Str::limit($shop->description ?? '-', 50)); ?>
                                ...
                                <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">
                                    <span class="inline-flex items-center gap-1.5 rounded-full bg-indigo-100 px-2.5 py-1 text-xs font-medium text-indigo-800 dark:bg-indigo-500/20 dark:text-indigo-200">
                                        ...
                                        <?php echo e($shop->products_count); ?>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <a href="<?php echo e(route('admin.sejajan.show', $shop)); ?>"
                                        ...
                                        <a href="<?php echo e(route('admin.sejajan.edit', $shop)); ?>"
                                           class="rounded-lg p-1.5 text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-white">
                                            <i class="fa-solid fa-pen text-xs"></i>
                                        </a>
                                        <form action="<?php echo e(route('admin.sejajan.destroy', $shop)); ?>" method="POST"
...
<?php endif; ?>
<?php /**PATH C:\LMSBACKEND\selaju-system\resources\views/admin/sejajan/index.blade.php ENDPATH**/ ?>