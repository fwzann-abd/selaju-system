...
<?php $component->withAttributes([]); ?>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <form action="<?php echo e(route('admin.classrooms.index')); ?>" method="GET" class="w-full md:max-w-sm">
                <div class="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm shadow-sm focus-within:border-indigo-500 dark:border-slate-700 dark:bg-slate-900">
                    ...
                    <input type="text" name="q" value="<?php echo e($search); ?>" placeholder="Cari nama, tingkat, jurusan, atau tahun ajaran"
                           class="w-full border-none bg-transparent text-sm text-slate-700 placeholder-slate-400 focus:ring-0 dark:text-slate-200"
                           autocomplete="off">
                    <?php if($search): ?>
                        <a href="<?php echo e(route('admin.classrooms.index')); ?>" class="text-xs text-indigo-500 hover:underline">Reset</a>
                    <?php endif; ?>
                </div>
            </form>
                ...
                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    <thead class="bg-slate-50 dark:bg-slate-800/60">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Tahun Ajaran</th>
                        ...
                        <?php $__empty_1 = true; $__currentLoopData = $classrooms; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $classroom): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="transition hover:bg-slate-50 dark:hover:bg-slate-800/60">
                                ...
                                <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
                                    <?php echo e($classroom->tingkat ?? '-'); ?>

                                </td>
                                <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
                                    <?php echo e($classroom->jurusan ?? '-'); ?>

                                </td>
                                <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
                                    <?php echo e($classroom->academic_year ?? '-'); ?>

                                </td>
                                <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
                                    <?php if($classroom->teacher): ?>
                                        <span class="inline-flex items-center rounded-full bg-indigo-100 px-3 py-1 text-xs font-medium text-indigo-800 dark:bg-indigo-500/20 dark:text-indigo-200">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <form action="<?php echo e(route('admin.classrooms.destroy', $classroom)); ?>" method="POST"
...
<?php endif; ?>
<?php /**PATH C:\LMSBACKEND\selaju-system\resources\views/admin/classrooms/index.blade.php ENDPATH**/ ?>