...
<?php $component->withAttributes([]); ?>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <form action="<?php echo e(route('admin.teachers.index')); ?>" method="GET" class="w-full md:max-w-2xl">
                <div class="grid gap-3 sm:grid-cols-2">
                    <div class="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm shadow-sm focus-within:border-indigo-500 dark:border-slate-700 dark:bg-slate-900">
                        ...
                        <input type="text" name="q" value="<?php echo e($search); ?>" placeholder="Cari nama, NIP, username, atau email"
                               class="w-full border-none bg-transparent text-sm text-slate-700 placeholder-slate-400 focus:ring-0 dark:text-slate-200"
                               autocomplete="off">
                        <?php if($search): ?>
                            <a href="<?php echo e(route('admin.teachers.index')); ?>" class="text-xs text-indigo-500 hover:underline">Reset</a>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-white px-3 py-2 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                        ...
                        <select name="school_id" class="w-full rounded-xl border border-slate-200 bg-transparent px-3 py-2 text-sm text-slate-700 focus:border-indigo-500 focus:outline-none dark:border-slate-700 dark:text-slate-200">
                            ...
                            <?php $__currentLoopData = $schools; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $school): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($school->id); ?>" <?php if($selectedSchool == $school->id): echo 'selected'; endif; ?>><?php echo e($school->name); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </div>
                </div>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white dark:divide-slate-800 dark:bg-slate-900">
                        <?php $__empty_1 = true; $__currentLoopData = $teachers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $teacher): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="transition hover:bg-slate-50 dark:hover:bg-slate-800/60">
                                <td class="px-6 py-4 text-sm font-semibold text-slate-800 dark:text-slate-100"><?php echo e($teacher->name); ?></td>
                                <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400"><?php echo e($teacher->nip ?? '-'); ?></td>
                                <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400"><?php echo e($teacher->school->name ?? '-'); ?></td>
                                <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400"><?php echo e($teacher->account->username ?? '-'); ?></td>
                                <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400"><?php echo e($teacher->account->email ?? '-'); ?></td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <form action="<?php echo e(route('admin.teachers.destroy', $teacher)); ?>" method="POST" onsubmit="return confirm('Yakin ingin menghapus guru ini?')">
                                            <?php echo csrf_field(); ?>
...
<?php endif; ?>
<?php /**PATH C:\LMSBACKEND\selaju-system\resources\views/admin/teachers/index.blade.php ENDPATH**/ ?>