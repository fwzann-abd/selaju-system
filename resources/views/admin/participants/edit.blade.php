        ...
        <form action="{{ route('admin.participants.update', $participant) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                    ...
                    <input id="username" name="username" type="text" value="{{ old('username', $participant->username) }}"
                           required
                ...
                <div>
                    ...
                    <input id="name" name="name" type="text" value="{{ old('name', $participant->name) }}"
                           required
                           class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-800 transition focus:border-indigo-500 focus:bg-white focus:dark:bg-slate-800 focus:ring-2 focus:ring-indigo-500/30 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                ...
                <div>
                    ...
                    <input id="no_telp" name="no_telp" type="text" value="{{ old('no_telp', $participant->no_telp) }}"
                           class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-800 transition focus:border-indigo-500 focus:bg-white focus:dark:bg-slate-800 focus:ring-2 focus:ring-indigo-500/30 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                ...
                <div>
                    ...
                    <input id="birth_date" name="birth_date" type="date" value="{{ old('birth_date', $participant->birth_date?->format('Y-m-d')) }}"
                           class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-800 transition focus:border-indigo-500 focus:bg-white focus:dark:bg-slate-800 focus:ring-2 focus:ring-indigo-500/30 dark:border-slate-700 dark:bg-slate-800 dark:text-white">
                    @error('birth_date')
                        <p class="mt-1 text-xs text-[#EF4444]">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="school_id" name="school_id"
                        ...
                        @foreach($schools as $school)
                            <option value="{{ $school->id }}" @selected(old('school_id', $participant->school_id) == $school->id)>
                                {{ $school->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label for="is_active" class="text-sm font-semibold text-slate-600 dark:text-slate-200">Status</label>
                    <select id="is_active" name="is_active"
                           class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-800 transition focus:border-indigo-500 focus:bg-white focus:dark:bg-slate-800 focus:ring-2 focus:ring-indigo-500/30 dark:border-slate-700 dark:bg-slate-800 dark:text-white">
                        <option value="1" @selected(old('is_active', $participant->is_active) == 1)>Aktif</option>
                        <option value="0" @selected(old('is_active', $participant->is_active) == 0)>Nonaktif</option>
                    </select>
                    @error('is_active')
                        <p class="mt-1 text-xs text-[#EF4444]">{{ $message }}</p>
                    @enderror
