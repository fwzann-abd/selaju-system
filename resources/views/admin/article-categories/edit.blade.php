<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-1">
            ...
            <h2 class="text-2xl font-semibold text-slate-900 dark:text-white">Edit Article Category</h2>
        </div>
    </x-slot>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <form action="#" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', 'Example Category') }}" required class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">Slug</label>
                    <input type="text" name="slug" value="{{ old('slug', 'example-category') }}" class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900" />
                </div>
