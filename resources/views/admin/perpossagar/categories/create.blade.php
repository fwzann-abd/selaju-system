    ...
    <x-slot name="header">
        <div class="flex flex-col gap-1">
            <nav class="text-sm text-slate-500 dark:text-slate-400">
                ...
                <span class="text-slate-900 dark:text-white">Tambah Kategori</span>
            </nav>
            <h2 class="text-2xl font-semibold text-slate-900 dark:text-white">Tambah Kategori Buku</h2>
        </div>
    </x-slot>

    <div class="space-y-6">
        <form action="{{ route('admin.perpossagar-categories.store') }}" method="POST" class="space-y-6">
            @csrf
