    ...
    <x-slot name="header">
        <div class="flex flex-col gap-1">
            <nav class="text-sm text-slate-500 dark:text-slate-400">
                ...
                <span class="text-slate-900 dark:text-white">Edit Buku</span>
            </nav>
            <h2 class="text-2xl font-semibold text-slate-900 dark:text-white">Edit Buku</h2>
        </div>
    </x-slot>

    <div class="space-y-6">
        <form action="{{ route('admin.perpossagar-books.update', $book->uuid) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                <textarea
                    ...
                    class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-slate-900 placeholder-slate-400 transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:focus:ring-indigo-900">{{ old('desc', $book->desc) }}</textarea>
            </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                <select
                    ...
                    required
                >
                    <option value="" disabled {{ old('language_id', $book->language_id) ? '' : 'selected' }}>Pilih bahasa buku...</option>
                    @foreach($languages as $language)
                        <option value="{{ $language->uuid }}" {{ old('language_id', $book->language_id) === $language->uuid ? 'selected' : '' }}>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                <div class="flex gap-3">
                    <input
                        ...
                        id="color_hex"
                        name="color_hex"
                        value="{{ old('color_hex', $book->color_hex ?? '#000000') }}"
                ...
                <div class="relative">
                    ...
                    <label for="photo" class="block cursor-pointer">
                        <div id="photoDropZone" class="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl p-8 text-center transition hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-950/20">
                            <div id="photoEmptyState" class="{{ $book->photo ? 'hidden' : '' }} space-y-3">
                            ...
                            <div id="photoPreviewState" class="{{ $book->photo ? '' : 'hidden' }} space-y-3">
                                <img id="photoPreview" src="{{ $book->photo ?? '' }}" alt="Preview" class="w-32 h-40 object-cover mx-auto rounded-lg">
                                <p id="photoFileName" class="text-sm font-medium text-slate-900 dark:text-white">{{ $book->photo ? basename($book->photo) : '' }}</p>
                    ...
                    <button
                        id="photoRemoveBtn"
                        type="button"
                        class="{{ $book->photo ? '' : 'hidden' }} absolute top-3 right-3 bg-red-500 hover:bg-red-600 text-white rounded-full p-2 transition"
                    >
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                @if ($book->filename)
                    <p class="mb-4 text-sm text-slate-600 dark:text-slate-400">
                        File saat ini: <span class="font-medium">{{ basename($book->filename) }}</span>
                    </p>
                @endif
                <input
                    type="file"
            ...
            <!-- Author Info -->
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-800/40">
                <p class="text-sm font-medium text-slate-900 dark:text-white mb-1">Penulis</p>
                <p class="text-sm text-slate-600 dark:text-slate-400">{{ $book->author?->participant->name ?? 'Unknown' }}</p>
                <p class="text-xs text-slate-500 dark:text-slate-500 mt-2">Penulis otomatis terdeteksi dari admin yang membuat buku</p>
            </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <label class="flex items-center gap-3 cursor-pointer">
                    ...
                    />
                    <span class="text-sm font-medium text-slate-700 dark:text-slate-300">Buku telah disetujui</span>
                </label>
            </div>
