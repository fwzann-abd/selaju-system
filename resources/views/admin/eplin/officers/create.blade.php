<x-app-layout>
    ...
    <div class="max-w-2xl">
        <form action="{{ route('admin.eplin.officers.store') }}" method="POST" class="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            @csrf

            <div>
                <label for="student_id" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Pilih Siswa <span class="text-red-500">*</span></label>
            ...
            <div>
                ...
                <select id="role" name="role" required
                        class="mt-2 block w-full rounded-lg border border-slate-300 bg-white px-4 py-2 text-slate-900 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 dark:border-slate-600 dark:bg-slate-800 dark:text-white">
                    <option value="petugas" @selected(old('role') === 'petugas' || !old('role'))>Petugas</option>
                </select>
                @error('role')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
