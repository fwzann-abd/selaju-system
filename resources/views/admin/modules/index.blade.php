        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                <p class="text-sm text-slate-600 dark:text-slate-300">
                    Total {{ $modules->total() }} module dalam sistem
                </p>
            </div>
                        ...
                        <tr class="transition hover:bg-slate-50 dark:hover:bg-slate-800/60">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="font-medium text-slate-900 dark:text-white">{{ $module->name }}</div>
                                        <div class="text-xs text-slate-500 dark:text-slate-400">{{ $module->identifiers }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-medium text-blue-800 dark:bg-blue-500/10 dark:text-blue-200">
                                    {{ $module->menu?->name ?? 'Tanpa Menu' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">
                                @if($module->url)
                                    <code class="rounded bg-slate-100 px-2 py-1 text-xs dark:bg-slate-800">{{ $module->url }}</code>
                                @else
