    ...
    <div class="space-y-6">
        <form action="{{ route('admin.menus.update', $menu) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            <input type="text" name="icon" id="icon" value="{{ old('icon', $menu->icon) }}" required
                                   class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 placeholder-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                            ...
                            <span class="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm">
                                <i id="icon-preview" class="fa-solid {{ old('icon', $menu->icon) ?? 'fa-cube' }} text-xl"></i>
                            </span>
                        </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="row_order" id="row_order" value="{{ old('row_order', $menu->row_order) }}" min="1" required
                               class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 placeholder-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="mt-2 space-y-2">
                            <div class="flex items-center">
                                <input type="radio" name="status" id="active" value="1" {{ old('status', $menu->status) == '1' ? 'checked' : ($menu->status ? 'checked' : '') }} class="h-4 w-4">
                                <label for="active" class="ml-2 block text-sm text-slate-700 dark:text-slate-300">Aktif</label>
                            </div>
                            <div class="flex items-center">
                                <input type="radio" name="status" id="inactive" value="0" {{ old('status', $menu->status) == '0' ? 'checked' : (!$menu->status ? 'checked' : '') }} class="h-4 w-4">
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm text-white">Simpan Perubahan</button>
            </div>
        </form>
    </div>
