{{-- Shared form partial for classroom create/edit --}}
{{-- @props: $action, $method, $buttonLabel, $classroom (nullable), $tingkatan, $jurusan, $academicYears, $teachers --}}
...
<div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <form action="{{ $action }}" method="POST"
          x-data="(function(){ return {
              ...
              tingkat: @js(old('tingkat', $classroom?->tingkat ?? '')),
              jurusan: @js(old('jurusan', $classroom?->jurusan ?? '')),
              academicYear: @js(old('academic_year', $classroom?->academic_year)) || `${new Date().getFullYear()}/${new Date().getFullYear() + 1}`,
              init() {},
              generateName() {
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div>
                ...
                <select id="tingkat" name="tingkat" 
                    ...
                    @foreach($tingkatan as $value => $label)
                        <option value="{{ $value }}" {{ old('tingkat', $classroom?->tingkat) == $value ? 'selected' : '' }} x-bind:selected="tingkat === '{{ $value }}'">
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select id="jurusan" name="jurusan" 
                    ...
                    @foreach($jurusan as $value => $label)
                        <option value="{{ $value }}" {{ old('jurusan', $classroom?->jurusan) == $value ? 'selected' : '' }} x-bind:selected="jurusan === '{{ $value }}'">
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select id="academic_year" name="academic_year" 
                    ...
                    @foreach($academicYears as $year)
                        <option value="{{ $year }}" {{ old('academic_year', $classroom?->academic_year) == $year ? 'selected' : '' }} x-bind:selected="academicYear === '{{ $year }}'">
                            {{ $year }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" id="rombel" name="rombel" value="{{ old('rombel', $classroom?->rombel) }}" 
