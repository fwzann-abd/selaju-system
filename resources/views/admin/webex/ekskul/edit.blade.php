<x-app-layout>
    ...
    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <form action="{{ route('admin.webex.ekskul.update', $ekskul) }}" method="POST"
              x-data="(function(){ return {
                  name: @js(old('name', $ekskul->name)),
                  slug: @js(old('slug', $ekskul->slug)),
            ...
            <div>
                ...
                <textarea id="bio" name="bio" rows="4" autocomplete="off"
                          class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-800 transition focus:border-indigo-500 focus:bg-white focus:dark:bg-slate-800 focus:ring-2 focus:ring-indigo-500/30 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                          placeholder="Jelaskan tentang ekskul ini...">{{ old('bio', $ekskul->bio) }}</textarea>
                @error('bio')
                    <p class="mt-1 text-xs text-[#EF4444]">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input id="logo" name="logo" type="url" autocomplete="off"
                       value="{{ old('logo', $ekskul->logo) }}"
                       class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-800 transition focus:border-indigo-500 focus:bg-white focus:dark:bg-slate-800 focus:ring-2 focus:ring-indigo-500/30 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                       placeholder="https://example.com/logo.jpg">
                @if ($ekskul->logo)
                    <div class="mt-3 p-3 rounded-2xl border border-slate-200 flex items-center gap-3 bg-slate-50 dark:border-slate-700 dark:bg-slate-800">
                        <img src="{{ $ekskul->logo }}" alt="{{ $ekskul->name }}" class="w-12 h-12 rounded object-cover">
